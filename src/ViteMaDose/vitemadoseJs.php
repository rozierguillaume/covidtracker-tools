...
const DEFAULT_VMD_URL = 'https://raw.githubusercontent.com/CovidTrackerFr/vitemadose/data-auto/data/output'
...
async function main() {
  const vmdUrl = getVmdUrl(window.location)
  const departements = await fetchDepartements(vmdUrl)
...
async function afficherLesSlotsPourDepartement(renduEl, départementSélectionné, vmdUrl) {
  ...
  const slots = await fetchSlotsForDepartement(vmdUrl, départementSélectionné.code_departement)
...
async function fetchDepartements (vmd_url) {
  const url = `${vmd_url}/departements.json`
...
async function fetchSlotsForDepartement(vmd_url, numeroDepartement) {
  const url = `${vmd_url}/${numeroDepartement}.json`
...
function getVmdUrl(location) {
  if (!location.search) {
    return DEFAULT_VMD_URL
  }
  const query = location.search.substr(1)
    .split('&')
    .map((kv) => kv.split('='))
    .map(([key, value]) => ({ [key]: value === undefined ? true : value }))
    .reduce((query, kv) => ({ ...kv, ...query }))

  if (query.vmd_url) {
    return query.vmd_url
  }
  return DEFAULT_VMD_URL
}
